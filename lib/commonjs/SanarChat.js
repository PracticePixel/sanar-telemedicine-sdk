  ...
  (0, _react.useEffect)(() => {
    if (props.enable) {
      if (_SanarTelemedicine.default.session) {
        const CHAT_URL = `${_SanarTelemedicine.default.session.chatUrl}/${notification.roomName}/${notification.uid}?token=${_SanarTelemedicine.default.session.token}&lang=${props.lang || 'en'}`;
        setBaseUrl(CHAT_URL);
      }
    }
  });
