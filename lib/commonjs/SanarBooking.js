...
const SanarBooking = props => {
  const [baseUrl, setBaseUrl] = (0, _react.useState)('');
  (0, _react.useEffect)(() => {
    if (props.enable) {
      if (_SanarTelemedicine.default.session) {
        setBaseUrl(_SanarTelemedicine.default.session.webUrl);
      }
    }
  });

  const onMessage = event => {
    console.log(event); // if ((!event.canGoBack && event.url==event.data) || (event.data && event.data.includes('home'))) {
    //     setBaseUrl('');
    //     props.onEndFlow();
    // }
...
//# sourceMappingURL=SanarBooking.js.map